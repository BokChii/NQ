...
export async function GET(request: Request) {
  ...
  async function getRedirectAfterAuth(defaultNext: string): Promise<string> {
    ...
    const { data: profile } = await supabase
      ...
      .eq("id", user.id)
      .maybeSingle();
    return profile?.onboarding_done ? defaultNext : "/onboarding";
  }
  ...
  if (token_hash && type) {
    ...
    const baseNext = next.startsWith("/") ? next : "/arena";
    const nextUrl = type === "recovery" ? baseNext : await getRedirectAfterAuth(baseNext);
    return NextResponse.redirect(new URL(nextUrl, request.url));
  }
